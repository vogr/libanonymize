...
int main(int argc, char **argv) {
  ...
  if(argc < 4) {
    std::cerr << "Usage\n\t";
    std::cerr << argv[0] << " <number of neighbors> <dimension of projection space> <treshold> [projection type|\"Gaussian\" or \"Rademacher\"]" << std::endl;
    return 1;
  }
...
*/
...

  {
    ...
    RMatrixXd datapoints_train = H5Easy::load<RMatrixXd>(hf_train, "representation");
    Eigen::VectorXi true_labels_train =  H5Easy::load<Eigen::VectorXi>(hf_train, "true_labels");
    
    H5Easy::File hf_test(testing_dset_fname_hdf5, H5Easy::File::ReadOnly);
    RMatrixXd datapoints_test = H5Easy::load<RMatrixXd>(hf_test, "representation");
    Eigen::VectorXi true_labels_test =  H5Easy::load<Eigen::VectorXi>(hf_test, "true_labels");
    t_datasets = clock() - t_datasets;
    std::cerr << "Execution time : " << (t_datasets*1000)/CLOCKS_PER_SEC <<"ms\n\n";
    ...
    clock_t t_knn_train_orig = clock();
    KnnClassificationBinary classifier_orig{K_NN, training_dataset_orig, threshold};
    t_knn_train_orig = clock() - t_knn_train_orig;
    std::cerr <<"Execution time: "
         <<(t_knn_train_orig*1000)/CLOCKS_PER_SEC
         <<"ms\n\n";
...

    {
        ...
        clock_t t_knn_test_orig = clock();
        for (int i = 0; i < n ; i++) {
          int l = classifier_orig.Estimate(datapoints_test.row(i));
    ...
    */
    ...
    clock_t t_knn_train_projected = clock();
    KnnClassificationBinary classifier_proj{K_NN, training_dataset_projected, threshold};
    t_knn_train_projected = clock() - t_knn_train_projected;
    std::cerr <<"Execution time: "
         <<(t_knn_train_projected*1000)/CLOCKS_PER_SEC
         <<"ms\n\n";
...

    {
        ...
        clock_t t_knn_test_projected = clock();
        for (int i = 0; i < n ; i++) {
          int l = classifier_proj.Estimate(testing_datapoints_projected.row(i));
          //std::cout << true_labels_test.coeff(i) << " -> " << l << std::endl;
          confusion_matrix.AddPrediction(true_labels_test(i), l);
        }
        t_knn_test_projected = clock() - t_knn_test_projected;
